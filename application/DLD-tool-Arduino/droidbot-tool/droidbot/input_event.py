...
KEY_RotationEvent = "rotation"
KEY_FillUIEvent = "fill_UI"
KEY_DoubleRotationEvent = "double_rotation",
KEY_WaitEvent = "wait_event"


TEXT_LENGTH_FOR_SET_TEXT_EVENT = 5
...
class InputEvent(object):
    ...
    def from_dict(event_dict):
        ...
        if event_type == KEY_KeyEvent:
            ...
            return TouchEvent(id=0, event_dict=event_dict)
        elif event_type == KEY_LongTouchEvent:
            return LongTouchEvent(id=0, event_dict=event_dict)
        elif event_type == KEY_SwipeEvent:
            return SwipeEvent(event_dict=event_dict)
        elif event_type == KEY_ScrollEvent:
            return ScrollEvent(id=0, event_dict=event_dict)
        elif event_type == KEY_SetTextEvent:
            return SetTextEvent(id=0, event_dict=event_dict)
        elif event_type == KEY_IntentEvent:
            return IntentEvent(intent=str(event_dict["intent"]), event_dict=event_dict)
        ...
        elif event_type == KEY_RotationEvent:
            return RotationEvent(event_dict=event_dict)
        elif event_type == "double_rotation":
            return DoubleRotationEvent(event_dict=event_dict)
        elif event_type == KEY_WaitEvent:
            return WaitEvent(event_dict=event_dict, time=event_dict["time"])
        elif event_type == KEY_FillUIEvent:
            return FillUIEvent(event_dict=event_dict)
...
class WaitEvent(InputEvent):
    ...
    def __init__(self, event_dict=None, time=5):
        self.event_type = KEY_WaitEvent
        if type(int(time)) == type(1): #it is a number...
            self.time = int(time)
        else:
            raise InvalidEventException("The argument type \"time\" for " + self.__class__.__name__ + " must be a number.")
        if event_dict is not None:
            self.__dict__.update(event_dict)
    ...
    def get_event_str(self, state):
        return "%s(time='%d')" % (self.__class__.__name__, int(self.time))
...
class KeyEvent(InputEvent):
    ...
    def create_unique_code(self):
        return utils.md5(self.__class__.__name__ + " " + self.name)
...
class UIEvent(InputEvent):
    ...
    def equals(self, other):
        ...
        if self.__class__.__name__ != other.__class__.__name__:
            return False
        if self.create_unique_code() != other.create_unique_code():
            return False
        return True
    ...
    def create_unique_code(self):
        ...
        code += str(self.id)
        code += str(self.view.get("class"))
        code += str(self.view.get("resource_id"))
        code += str(self.view.get("clickable"))
        code += str(self.view.get("checkable"))
        code += str(self.view.get("long_clickable"))
        code += str(self.view.get("scrollable"))
        code += str(self.view.get("editable"))
        code += str(self.view.get("visible"))
        code += str(self.view.get("enabled"))
        return utils.md5(code)
...
class TouchEvent(UIEvent):
    ...
    def __init__(self, id=None, x=None, y=None, view=None, event_dict=None):
        super(TouchEvent, self).__init__(id)
...
class LongTouchEvent(UIEvent):
    ...
    def __init__(self, id=None, x=None, y=None, view=None, duration=2000, event_dict=None):
        super(LongTouchEvent, self).__init__(id)
...
class SwipeEvent(UIEvent):
    ...
    def __init__(self,
        ...
        super(UIEvent, self).__init__(id)
...
class ScrollEvent(UIEvent):
    ...
    def __init__(self, id=None, x=None, y=None, view=None, direction="DOWN", event_dict=None, y_full_down=1750):
        super(ScrollEvent, self).__init__(id)
        self.event_type = KEY_ScrollEvent
        self.x = x
        self.y = y
        self.view = view
        self.y_full_down = y_full_down
    ...
    def create_unique_code(self):
        if self.view is not None:
            return super(UIEvent, self).create_unique_code()
        else:
            return utils.md5(self.__class__.__name__ + " " + self.direction)
    ...
    def send(self, device):
        ...
        if self.direction == "UP":
            ...
            start_x += width * 2 / 5
            end_x -= width * 2 / 5
        elif self.direction == "FULL_DOWN":
            start_y = self.y_full_down
            end_y = 250
            duration = 2500
            start_x = end_x = 540
        elif self.direction == "FULL_LEFT":
            start_y = end_y = 960
            duration = 2500
            start_x = 5
            end_x = 1075
        elif self.direction == "FULL_RIGHT":
            start_y = end_y = 960
            duration = 2500
            start_x = 1075
            end_x = 5
    ...
    def get_event_str(self, state):
        return "%s(direction=%s)" % (self.__class__.__name__, self.direction)
...
class SetTextEvent(UIEvent):
    ...
    def __init__(self, id=None, x=None, y=None, view=None, text="test123", event_dict=None):
        super(SetTextEvent, self).__init__(id)
    ...
    def send(self, device):
        ...
        device.view_set_text(new_text=self.text)
        return True
...
class SpawnEvent(InputEvent):
    ...
    def get_event_str(self, state):
        return "%s(state=%s)" % self.__class__.__name__, state.state_str


class FillUIEvent(InputEvent):

    clickable_classes = ["android.widget.CheckBox", "android.widget.RadioButton", "android.widget.Switch",
                         "android.widget.ToggleButton"]
    editable_classes = ["android.widget.EditText"]

    def __init__(self, event_dict=None, views=[]):
        self.event_type = KEY_FillUIEvent
        self.views = []
        self.views_str = []
        for view in views:
            if view["class"] in FillUIEvent.editable_classes + FillUIEvent.clickable_classes:
                self.views.append(view)
                self.views_str.append(view["view_str"])
        if event_dict is not None:
            self.__dict__.update(event_dict)
    ...
    def send(self, device):
        ...
        for view_dict in self.views:
            if view_dict["class"] in self.editable_classes and DeviceState.is_view_in_the_screen(device, view_dict):
                SetTextEvent(view=view_dict).send(device)
            elif view_dict["class"] in self.clickable_classes and DeviceState.is_view_in_the_screen(device, view_dict):
                TouchEvent(view=view_dict).send(device)
        return True
    ...
    def get_views(self):
        return self.views if self.views else []


class RotationEvent(InputEvent):
    """
    an event to change the orientation of the device
    """

    def __init__(self, event_dict=None, view=None, direction="RIGHT"):
        self.event_type = KEY_RotationEvent
        self.view = view
        self.direction = direction
        if event_dict is not None:
            self.__dict__.update(event_dict)
    ...
    def get_random_instance(device, app):
        ...
        return RotationEvent(direction=direction)

    @staticmethod
    def restore_default_orientation(device):
        device.adb.rotate(0)
    ...
    def get_event_str(self, state):
        return "%s()" % self.__class__.__name__, state.state_str

    def get_views(self):
        return [self.view] if self.view else []


class DoubleRotationEvent(InputEvent):
    """
    an event to change the orientation of the device twice
    """

    def __init__(self, event_dict=None, view=None):
        self.event_type = KEY_DoubleRotationEvent
        self.view = view
        if event_dict is not None:
            self.__dict__.update(event_dict)

    def create_unique_code(self):
        return utils.md5(self.__class__.__name__)
...
EVENT_TYPES = {
    ...
    KEY_FillUIEvent: FillUIEvent,
    KEY_WaitEvent: WaitEvent,
    KEY_DoubleRotationEvent: DoubleRotationEvent
}
