...
class DeviceState(object):
    ...
    def __init__(self, device, views, foreground_activity, activity_stack, background_services,
        ...
        self.views = DeviceState.__fix_passwords(self.views)
        self.view_tree = {}
    ...
    @staticmethod
    def __fix_passwords(views_list):
        bullet = u'\u2022'
        for view_dict in views_list:
            if view_dict["is_password"]:
                text = view_dict["text"]
                if text is None:
                    continue
                tmp_arr = text.split(bullet)
                tmp_str = ""
                for i in range(0, len(tmp_arr)):
                    tmp_str += bullet
                    if tmp_arr[i] != "":
                        tmp_str += bullet
                view_dict["text"] = tmp_str
        return views_list
    ...
    def __assemble_view_tree(self, root_view, views):
        if not len(self.view_tree) and len(views) > 0: # bootstrap
            self.view_tree = copy.deepcopy(views[0])
            self.__assemble_view_tree(self.view_tree, views)
        else:
            if root_view.get("children") is None:
                return
    ...
    def get_possible_input(self):
        ...
        # if the possible events are already checked, it returns immediately the list of possible events
        # otherwise it creates this list
        ...
        for view_dict in self.views:
            ...
            if self.__safe_dict_get(view_dict, 'enabled') and \
               self.__safe_dict_get(view_dict, 'resource_id') != 'android:id/navigationBarBackground' and self.__safe_dict_get(view_dict, 'visible'):
                # temp_id is the position of the views found
                enabled_view_ids.append(view_dict['temp_id']) #lista di id delle view
        enabled_view_ids.reverse()

        event_id = 0
        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'clickable') and DeviceState.is_view_in_the_screen(self.device, self.views[view_id]):
                possible_events.append(TouchEvent(id=event_id, view=self.views[view_id]))
                event_id += 1
                touch_exclude_view_ids.add(view_id)
                touch_exclude_view_ids.union(self.get_all_children(self.views[view_id]))

        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'scrollable') and DeviceState.is_view_in_the_screen(self.device, self.views[view_id]):
                possible_events.append(ScrollEvent(id=event_id, view=self.views[view_id], direction="UP"))
                event_id += 1
                possible_events.append(ScrollEvent(id=event_id, view=self.views[view_id], direction="DOWN"))
                event_id += 1
                possible_events.append(ScrollEvent(id=event_id, view=self.views[view_id], direction="LEFT"))
                event_id += 1
                possible_events.append(ScrollEvent(id=event_id, view=self.views[view_id], direction="RIGHT"))
                event_id += 1


        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'checkable') and DeviceState.is_view_in_the_screen(self.device, self.views[view_id]):
                possible_events.append(TouchEvent(id=event_id, view=self.views[view_id]))
                event_id += 1
                touch_exclude_view_ids.add(view_id)
                touch_exclude_view_ids.union(self.get_all_children(self.views[view_id]))
        ...
        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'long_clickable') and DeviceState.is_view_in_the_screen(self.device, self.views[view_id]):
                possible_events.append(LongTouchEvent(id=event_id,  view=self.views[view_id]))
                event_id += 1

        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'editable') and DeviceState.is_view_in_the_screen(self.device, self.views[view_id]):
                possible_events.append(SetTextEvent(id=event_id, view=self.views[view_id], text="HelloWorld"))
                event_id += 1
                touch_exclude_view_ids.add(view_id)
        ...
        for view_id in enabled_view_ids:
            ...
            if children and len(children) > 0:
                continue
            if DeviceState.is_view_in_the_screen(self.device, self.views[view_id]):
                possible_events.append(TouchEvent(id=event_id, view=self.views[view_id]))
                event_id += 1
    ...
    def get_possible_input_for_dataloss_policy(self):
        possible_events = []
        enabled_views = []
        views = View.filter_views(self.views)
        for view_dict in views:
            # exclude navigation bar if exists
            if self.__safe_dict_get(view_dict, 'enabled') and \
                    self.__safe_dict_get(view_dict, 'resource_id') \
                    != 'android:id/navigationBarBackground' and self.__safe_dict_get(view_dict, 'visible'):
                # temp_id is the position of the views found
                enabled_views.append(view_dict)  # lista di id delle view

        event_id = 0
        for view in enabled_views:
            if self.__safe_dict_get(view, 'clickable') and DeviceState.is_view_in_the_screen(self.device, view):
                possible_events.append(TouchEvent(id=event_id, view=view))
                event_id += 1

        for view in enabled_views:
            if self.__safe_dict_get(view, 'scrollable') and DeviceState.is_view_in_the_screen(self.device, view):
                possible_events.append(ScrollEvent(id=event_id, view=view, direction="UP"))
                event_id += 1
                possible_events.append(ScrollEvent(id=event_id, view=view, direction="DOWN"))
                event_id += 1
                possible_events.append(ScrollEvent(id=event_id, view=view, direction="LEFT"))
                event_id += 1
                possible_events.append(ScrollEvent(id=event_id, view=view, direction="RIGHT"))
                event_id += 1

        for view in enabled_views:
            if self.__safe_dict_get(view, 'checkable') and DeviceState.is_view_in_the_screen(self.device, view):
                possible_events.append(TouchEvent(id=event_id, view=view))
                event_id += 1

        for view in enabled_views:
            if self.__safe_dict_get(view, 'long_clickable') and DeviceState.is_view_in_the_screen(self.device, view):
                possible_events.append(LongTouchEvent(id=event_id, view=view))
                event_id += 1

        for view in enabled_views:
            if self.__safe_dict_get(view, 'editable') and DeviceState.is_view_in_the_screen(self.device, view):
                possible_events.append(SetTextEvent(id=event_id, view=view, text="HelloWorld"))
                event_id += 1

        self.possible_events = possible_events
        possible_events.append(KeyEvent(name="BACK"))
        possible_events.append(ScrollEvent(direction="FULL_LEFT"))  # used to open lateral menu
        possible_events.append(ScrollEvent(direction="FULL_RIGHT"))

        return [] + possible_events

    @staticmethod
    def is_view_in_the_screen(device, view):
        # this method checks if the view passed as argument is in the current screen
        if view.get("bounds") is None:
            return False
        bounds = view["bounds"]
        if device.interactive_coordinates["MIN_X"] <= bounds[0][0] <= device.interactive_coordinates["MAX_X"] and \
                device.interactive_coordinates["MIN_Y"] <= bounds[0][1] <= device.interactive_coordinates["MAX_Y"] and \
                device.interactive_coordinates["MIN_X"] <= bounds[1][0] <= device.interactive_coordinates["MAX_X"] and \
                device.interactive_coordinates["MIN_Y"] <= bounds[1][1] <= device.interactive_coordinates["MAX_Y"]:
            return True
        return False