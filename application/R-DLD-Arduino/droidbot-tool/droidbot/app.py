...
class ListOfActivitiesNotValidException(Exception):
    pass
...
class App(object):
    ...
    def get_activity_coverage_percentage(self, activities_already_covered):
        if len(activities_already_covered) != len(set(activities_already_covered)):
            raise ListOfActivitiesNotValidException("There is a duplicated activity in activities_already_covered")

        for activity in activities_already_covered:
            if activity not in self.activities:
                raise ListOfActivitiesNotValidException("activities_already_covered must \
                                                        contain only target app's activities")

        return round(len(activities_already_covered) * 100 / len(self.activities), 2)
