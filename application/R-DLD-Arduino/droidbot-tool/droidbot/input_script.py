...
class ViewSelector(object):
    ...
    selector_grammar = {
        ...
        'x_up_left': STR_VAL,
        'y_up_left': STR_VAL,
        'x_down_right': STR_VAL,
        'y_down_right': STR_VAL,
        'focused': STR_VAL
    }
    ...
    def __init__(self, view_selector_id, selector_dict, script):
        ...
        self.x_up_left = None
        self.y_up_left = None
        self.x_down_right = None
        self.y_down_right = None
        self.focused = None
        self.parse()
    ...
    def parse(self):
        ...
        for selector_key in self.selector_dict:
            ...
            selector_value = str(self.selector_dict[selector_key])
            ...
            elif selector_key == 'x_up_left':
                self.x_up_left = int(selector_value)
            elif selector_key == 'y_up_left':
                self.y_up_left = int(selector_value)
            elif selector_key == 'x_down_right':
                self.x_down_right = int(selector_value)
            elif selector_key == 'y_down_right':
                self.y_down_right = int(selector_value)
            elif selector_key == 'focused':
                self.focused = eval(selector_value)
    ...
    def match(self, view_dict):
        ...
        if self.class_re and not safe_re_match(self.class_re, view_dict['class']):
            return False
        if self.focused and self.focused != view_dict['focused']:
            return False
        ...
        for (x, y) in self.out_coordinates:
            if bound_x_min < x < bound_x_max and bound_y_min < y < bound_y_max:
                return False
        if self.x_up_left and self.x_up_left != bound_x_min:
            return False
        if self.y_up_left and self.y_up_left != bound_y_min:
            return False
        if self.x_down_right and self.x_down_right != bound_x_max:
            return False
        if self.y_down_right and self.y_down_right != bound_y_max:
            return False
...
class ScriptEvent():
    ...
    def get_transformed_event(self, input_policy):
        ...
        if event_dict.get('event_type') == 'spawn':
